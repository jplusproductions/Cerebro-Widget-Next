...
export default function PortalPlayersPage() {
  ...
  // Players autocomplete query — fetches top 3 results when user types
  ...
  // Players list query — uses appliedSearch so it only re-fetches on Apply
  ...
  const suggestions = suggestionsData?.players ?? []
  ...
  return (
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        {/* Search with player autocomplete */}
                ...
                {suggestions.map((player) => (
                  <li key={player.id}>
                    <button
                      type="button"
                      onClick={() => {
                        router.push(`/portal/players/${player.id}`)
                      }}
                    ...
                    >
                      ...
                      {player.teamName && (
                        <span className="ml-auto shrink-0 text-xs text-zinc-400">{player.teamName}</span>
                      )}
                    </button>
                  </li>
                ))}
      ...
      <Table
        ...
        onRowClick={(player) => router.push(`/portal/players/${player.id}`)}
      />
    </div>
  )
}
